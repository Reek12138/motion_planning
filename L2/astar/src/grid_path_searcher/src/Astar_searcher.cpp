    ...
    for(int i = 0; i < GLX_SIZE; i++){
        ...
        for(int j = 0; j < GLY_SIZE; j++){
            ...
            for( int k = 0; k < GLZ_SIZE;k++){
                ...
                GridNodeMap[i][j][k] = std::make_shared<GridNode>(tmpIdx, pos);
...
//     Vector3i newIdx = currentPtr->index + di;

//     if(newIdx[0] >= 0 && newIdx[0] < GLX_SIZE &&
//        newIdx[1] >= 0 && newIdx[1] < GLY_SIZE &&
//        newIdx[2] >= 0 && newIdx[2] < GLZ_SIZE ){

//         if(!isOccupied(newIdx)){
//             GridNodePtr neighborNode = GridNodeMap[newIdx[0]][newIdx[1]][newIdx[2]];
//             neighborPtrSets.push_back(neighborNode);
//             edgeCostSets.push_back(1.0);
//         }
//        } 

//    }
    Eigen::Vector3i thisindex = currentPtr->index;
    int x = thisindex[0];
    int y = thisindex[1];
    int z = thisindex[2];
        ...
        for(int j=y-1;j<=y+1;j++){
            for(int k=z-1;k<=z+1;k++){
                ...
                if( i >= 0 && i <= GLX_SIZE &&
                    j >= 0 && j <= GLY_SIZE &&
                    k >= 0 && k <= GLZ_SIZE){
                        if(!isOccupied(Eigen::Vector3i(i,j,k))){
                            GridNodePtr neighborPtr = GridNodeMap[i][j][k];
                            // neighborPtr = GridNodeMap[i][j][k];

                            double edgeCost = std::sqrt(std::pow(i - x, 2) + 
                                std::pow(j - y, 2) + std::pow(k - z, 2));
...
{   
    ...
    GridNodePtr startPtr = std::make_shared<GridNode>(start_idx, start_pt);
    GridNodePtr endPtr = std::make_shared<GridNode>(end_idx, end_pt);
    ...
    *
    */
    // initGridMap(resolution, global_xyz_lower, global_xyz_upper, max_x_id, max_y_id, max_z_id);
